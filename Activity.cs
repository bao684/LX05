...
namespace DAL
{
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity> GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select *from activity where activityVerify='审核通过'and recommend='是' and endTime>=now() order by endTime desc limit 2";
        ...
        public int Delete(int id)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "detele from activity where activityid=@id";
        ...
        public int UpdateRecommend(Model.Activity activity)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update activity set arecommend=@recommend,recommendTime=recommendTime where activityid=@activityid";
